...
namespace persee {
  ...
  FrameRef convertTo255bitGrayscaleData(int texWidth, int texHeight, FrameRef ref) {
    return convertTo255bitGrayscaleData(texWidth, texHeight, ref->data());
  }

  /// Generates converter lambda for specified width/height
  std::function<FrameRef(const void*, size_t)> grayscale255bitConverter(int texWidth, int texHeight) {
    return [texWidth, texHeight](const void* data, size_t size){
      return convertTo255bitGrayscaleData(texWidth, texHeight, data);
    };
  }
}
