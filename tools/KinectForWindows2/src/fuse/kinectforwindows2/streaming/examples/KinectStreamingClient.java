...
import fuse.kinectforwindows2.streaming.KinectSkeletonReceiver;
...
public class KinectStreamingClient implements GLEventListener, KeyListener, KinectStreamListener
{
	...
	private KinectSkeletonReceiver skeletonReceiver;
	private long frameTime, lifetime, maxFrameTime;
...
//    	gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, GL2.GL_RGBA, 512, 424, 0, GL2.GL_RGBA, GL2.GL_UNSIGNED_BYTE, kinectBuffer);
		...
		{
			long time = skeletonReceiver.skeletons().get(skeleton);
		...
		textRenderer.draw("TRACKED SKELETONS: " + count, 10, 10);
